...
//         {/* Show Spotify Preview if URL is valid */}
...
//             {...imageReg}
//             ref={(e) => {
//               if (imageReg && typeof imageReg.ref === "function") imageReg.ref(e);
...
function CardStep2({
  ...
  const handleFiles = useCallback(
    (files) => {
      ...
      if (typeof setValue === "function") {
        setValue("image", file, { shouldDirty: true, shouldTouch: true });
        // ✅ Store a URL version for MusicCard
    ...
    if (isRecording) {
      ...
      try {
        ...
        mediaRecorder.onstop = () => {
          ...
          const audioURL = URL.createObjectURL(audioBlob);
          setAudioUrl(audioURL);
          setValue("voiceUrl", audioURL, { shouldDirty: true }); // ✅ store URL for MusicCard
          setValue("voiceNote", audioBlob, { shouldDirty: true });
        };
