  ...
  return (
    <div className="media-step">
      ...
      <div className="form-group">
        ...
        <input type="text" placeholder="Paste Spotify URL" className="input-field" value={musicUrl} onChange={(e) => setValue("musicUrl", e.target.value)} />
        ...
        {getSpotifyEmbedUrl(musicUrl) && (
          <div className="spotify-preview" style={{ marginTop: "10px" }}>
            <iframe src={getSpotifyEmbedUrl(musicUrl)} width="100%" height="80" frameBorder="0" allow="encrypted-media" title="Spotify Preview"></iframe>
          </div>
        )}
      </div>
        ...
        <div className="voice-container">
          <div className="voice-left">
            <div className="voice-icon" aria-hidden>
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" style={{ width: "2rem" }}>
          ...
          <button type="button" className={`record-btn ${isRecording ? "recording" : ""}`} onClick={handleRecord}>
            {isRecording ? "Stop" : "Record"}
          </button>
        </div>
        {audioUrl && <div className="audio-preview" style={{ marginTop: "10px" }}><audio controls src={audioUrl}></audio></div>}
      </div>

      {/* 🖼️ Image */}
      <div className="form-group">
        <label>Add Image</label>
        <div className={`image-dropzone ${dragActive ? "drag-active" : ""}`} onDragOver={onDragOver} onDragLeave={onDragLeave} onDrop={onDrop} onClick={() => fileInputRef.current?.click()} role="button" tabIndex={0}>
          <div className="image-icon" aria-hidden>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" style={{ width: "2rem" }}>
              <path strokeLinecap="round" strokeLinejoin="round" d="m2.25 15.75 5.159-5.159a2.25 2.25 0 0 1 3.182 0l5.159 5.159m-1.5-1.5 1.409-1.409a2.25 2.25 0 0 1 3.182 0l2.909 2.909m-18 3.75h16.5a1.5 1.5 0 0 0 1.5-1.5V6a1.5 1.5 0 0 0-1.5-1.5H3.75A1.5 1.5 0 0 0 2.25 6v12a1.5 1.5 0 0 0 1.5 1.5Z" />
            </svg>
          </div>
          <p className="image-text">Drag and drop your image here<br/><span className="muted">Support JPEG, PNG, GIF Max (5MB)</span></p>
          <button type="button" className="browse-btn" onClick={(e) => { e.stopPropagation(); fileInputRef.current?.click(); }}>Browse Files</button>
          <input type="file" accept="image/*" className="hidden-file-input" {...imageReg} ref={(e) => { if (imageReg?.ref) imageReg.ref(e); fileInputRef.current = e; }} onChange={(e) => { imageReg?.onChange?.(e); onFileChange(e); }} />
          {previewUrl && <div className="image-preview"><img src={previewUrl} alt="Preview" style={{ width: "100%", borderRadius: "8px", marginTop: "10px" }} /></div>}
        </div>
      ...
      <div className="form-group">
        ...
        <input type="text" placeholder="Enter recipient name" className="input-field" value={recipient} onChange={(e) => setValue("recipient", e.target.value)} style={{ fontFamily: selectedFont || "inherit", fontSize: selectedFontSize || "16px" }} />
      </div>

      {/* ✍️ Quote */}
      <div className="form-group">
        <label>Add Favorite Quote</label>
        <input type="text" placeholder="Enter quote" className="input-field" value={quote} onChange={(e) => setValue("quote", e.target.value)} style={{ fontFamily: selectedFont || "inherit", fontSize: selectedFontSize || "16px" }} />
      </div>
